...
# -*- coding: utf-8 -*-
"""
Copied over from https://github.com/fenchri/edge-oriented-graph/blob/master/data_processing/
...
pwd = subprocess.check_output(["pwd"], shell=True).decode("utf-8")


genia_splitter = os.path.join("./scripts/data/common", "geniass")
genia_tagger = GENIATagger(os.path.join("./scripts/data/common", "genia-tagger-py", "geniatagger-3.0.2", "geniatagger"))
...
def sentence_split_genia(tabst):
    ...
    # os.popen('geniass temp_file.txt temp_file.split.txt > /dev/null 2>&1').read()
    output = subprocess.check_output(["geniass temp_file.txt temp_file.split.txt > /dev/null 2>&1"], shell=True)
