...
class DocumentRelationExtractor(Model):
    ...
    def __init__(self,
        ...
        self._namespaces = [entry for entry in vocab.get_namespaces() if "document_relations_labels" in entry]
    ...
    def forward(self,  # type: ignore
        ...
        """
        self._active_namespace = f"{metadata.dataset}__document_relations_labels"
    ...
    def predict(self, top_spans, relation_scores, num_spans_to_keep, metadata):
        ...
        pred_dict_doc, predictions_doc = self._predict_document(
            top_spans[0], relation_scores[0], num_spans_to_keep[0], document)
            
        # predictions.append(predictions_doc)

        return pred_dict_doc, predictions_doc

    def _predict_document(self, top_spans, relation_scores, num_spans_to_keep, document):
        ...
        for i, j in ix.nonzero(as_tuple=False):
            ...
            predictions.append(document.PredictedDocumentRelation(list_entry, document.sentences, sentence_offsets=True))
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        for name in ["precision", "recall", "f1"]:
            ...
            res_avg[f"MEAN__document_relations_{name}"] = sum(values) / len(values) if values else 0
            res.update(res_avg)
